...
from src.train_final import LinguisticFeatures

def load_artifacts():
    """Load the final optimized model and preprocessing pipeline"""
    try:
        model = joblib.load("models/model_final.pkl")
        feature_pipeline = joblib.load("models/feature_pipeline_final.pkl")
        label_encoder = joblib.load("models/label_encoder_final.pkl")
        optimal_threshold = joblib.load("models/optimal_threshold_final.pkl")
        return model, feature_pipeline, label_encoder, optimal_threshold
    except FileNotFoundError:
        print("Final model files not found. Please run train_final.py first.")
