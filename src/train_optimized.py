...
class LinguisticFeatures(BaseEstimator, TransformerMixin):
    """Extract linguistic features from text - optimized for real news detection"""
    ...
    def extract_features(self, text):
        ...
        positive_words = {'good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'positive', 'success', 'win', 'victory', 'best', 'better', 'improve', 'increase', 'rise', 'gain', 'development', 'progress', 'achievement'}
        negative_words = {'bad', 'terrible', 'awful', 'horrible', 'negative', 'fail', 'loss', 'defeat', 'crisis', 'problem', 'worst', 'worse', 'decline', 'decrease', 'fall', 'drop', 'disaster', 'scandal'}
        ...
        fake_indicators = {'breaking', 'shocking', 'unbelievable', 'exclusive', 'urgent', 'alert', 'warning', 'conspiracy', 'cover-up', 'scandal', 'exposed', 'revealed', 'leaked', 'bombshell', 'explosive'}
        ...
        professional_words = {'according', 'reported', 'official', 'statement', 'announced', 'confirmed', 'data', 'research', 'study', 'analysis', 'government', 'department', 'ministry'}
...
def find_optimal_threshold(y_true, y_proba):
    """Find optimal threshold for balanced precision and recall"""
...
def main(args):
    ...
    # Enhanced models with optimized class weights for better real news detection
    # Note: class_weight uses numeric labels (0=fake, 1=real)
    models = {
        "logistic": LogisticRegression(
            max_iter=2000, 
            class_weight={0: 0.9, 1: 1.1},  # Favor real news slightly (1=real)
            solver="saga", 
            n_jobs=-1, 
    ...
    for name, model in models.items():
        ...
        if hasattr(model, 'predict_proba'):
            ...
            optimal_threshold, optimal_f1 = find_optimal_threshold(y_val, y_proba)
    ...
    joblib.dump(le, "models/label_encoder_optimized.pkl")
    ...
    plot_confusion_matrix(cm, list(le.classes_), "reports/confusion_matrix_optimized.png")
    
    # Save detailed metrics
    with open("reports/metrics_optimized.json", "w") as f:
        json.dump({
            "results": results, 
