...
class LinguisticFeatures(BaseEstimator, TransformerMixin):
    """Extract linguistic features from text - heavily optimized for real news detection"""
    ...
    def extract_features(self, text):
        ...
        # Optimized sentiment indicators
        ...
        # Strong fake news indicators (most important)
        fake_indicators = {'breaking', 'shocking', 'unbelievable', 'exclusive', 'urgent', 'alert', 'warning', 'conspiracy', 'cover-up', 'scandal', 'exposed', 'revealed', 'leaked', 'bombshell', 'explosive', 'outrageous', 'incredible', 'amazing', 'unprecedented'}
        ...
        professional_words = {'according', 'reported', 'official', 'statement', 'announced', 'confirmed', 'data', 'research', 'study', 'analysis', 'government', 'department', 'ministry', 'institution', 'organization', 'agency', 'commission', 'committee', 'council', 'authority'}
        features['professional_word_ratio'] = sum(1 for word in words if word in professional_words) / len(words) if words else 0
        
        # Formal language indicators
        formal_words = {'however', 'therefore', 'furthermore', 'moreover', 'consequently', 'nevertheless', 'additionally', 'specifically', 'particularly', 'especially', 'significantly', 'substantially', 'considerably'}
        ...
        source_words = {'source', 'sources', 'official', 'spokesperson', 'representative', 'spokesman', 'spokeswoman', 'spokespeople', 'authority', 'expert', 'specialist', 'analyst', 'researcher', 'scientist'}
...
def find_optimal_threshold(y_true, y_proba, target_recall=0.8):
    """Find optimal threshold that prioritizes real news recall"""
    precision, recall, thresholds = precision_recall_curve(y_true, y_proba)
    
    # Find threshold that gives us good recall for real news (class 1)
    # We want to minimize false negatives for real news
    real_recall_indices = np.where(recall >= target_recall)[0]
    
    if len(real_recall_indices) > 0:
        # Choose the threshold that gives us the best precision while maintaining target recall
        best_idx = real_recall_indices[np.argmax(precision[real_recall_indices])]
        optimal_threshold = thresholds[best_idx] if best_idx < len(thresholds) else 0.3
    else:
        # Fallback: find threshold that maximizes F1 score
...
def main(args):
    ...
    # Enhanced models with heavily optimized class weights for real news detection
    models = {
        "logistic": LogisticRegression(
            max_iter=2000, 
            class_weight={0: 0.7, 1: 1.3},  # Heavily favor real news
            solver="saga", 
            n_jobs=-1, 
    ...
    for name, model in models.items():
        ...
        if hasattr(model, 'predict_proba'):
            ...
            # Find optimal threshold with focus on real news recall
            optimal_threshold = find_optimal_threshold(y_val, y_proba, target_recall=0.8)
    ...
    joblib.dump(feature_pipeline, "models/feature_pipeline_final.pkl")
    joblib.dump(best_model, "models/model_final.pkl")
    joblib.dump(le, "models/label_encoder_final.pkl")
    joblib.dump(best_threshold, "models/optimal_threshold_final.pkl")
    ...
    plot_confusion_matrix(cm, list(le.classes_), "reports/confusion_matrix_final.png")
